...
//$getgamepass = 'SELECT gamepass FROM gameadmin WHERE id = 1';
//$gamepass = $con->query('SELECT gamepass FROM gameadmin WHERE id = 1');

if ($stmt = $con->prepare('SELECT gamepass FROM gameadmin WHERE id = 1')) {
    $stmt->execute();
    $stmt->bind_result($gamepass);
    var_export($gamepass);
    $stmt->close();
} else {
    // Something is wrong with the sql statement, check to make sure players table exists with all fields.
    echo 'Could not sort out Game Pass!';
}
